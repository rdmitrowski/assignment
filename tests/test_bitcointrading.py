...
def load_dataframe() -> DataFrame:
    return spark.createDataFrame(data=TEST_DATA, schema=TEST_COLUMNS)
    
    
def test_dataset_filtering(dataset_filtering_data):
    ...
    assert_df_equality(bt.dataset_filtering(load_dataframe(), "id in (10, 50)"), df_expected)


def test_column_remove(dataset_filtering_data: DataFrame):
    expected_data = [(10, 1), (20, 2), (30, 3), (40, 4), (50, 5)]
    expected_columns = ["id", "value"]
    df_expected = spark.createDataFrame(data=expected_data, schema=expected_columns)
    assert_df_equality(bt.column_remove(load_dataframe(), "seq_number"), df_expected)


def test_column_rename(dataset_filtering_data: DataFrame):
    expected_data = [(10, "a", 1), (20, "b", 2), (30, "c", 3), (40, "d", 4), (50, "e", 5)]
    expected_columns = ["id", "incemental_id", "value"]
    df_expected = spark.createDataFrame(data=expected_data, schema=expected_columns)
    assert_df_equality(bt.column_rename(load_dataframe(), {'seq_number': 'incemental_id'}), df_expected)


def test_column_create(dataset_filtering_data: DataFrame):
    ...
    df_expected = spark.createDataFrame(data=expected_data, schema=expected_columns)
    assert_df_equality(bt.column_create(load_dataframe(), "multiplication", concat(col("id"), lit(' '), col("value"))), df_expected)
