...
def test_dataset_filtering(dataset_filtering_data: DataFrame):
    ...
    assert_df_equality(bt.dataset_filtering(dataset_filtering_data, "id in (10, 50)"), df_expected)
...
def test_column_remove(dataset_filtering_data: DataFrame):
    ...
    # df_users = spark.createDataFrame(data=input_data, schema=input_columns)
    df_expected = spark.createDataFrame(data=expected_data, schema=expected_columns)
    assert_df_equality(bt.column_remove(dataset_filtering_data, "seq_number"), df_expected)
...
def test_column_rename(dataset_filtering_data: DataFrame):
    ...
    #df_users = spark.createDataFrame(data=input_data, schema=input_columns)
    df_expected = spark.createDataFrame(data=expected_data, schema=expected_columns)
    assert_df_equality(bt.column_rename(dataset_filtering_data, {'seq_number': 'incemental_id'}), df_expected)
...
def test_column_create(dataset_filtering_data: DataFrame):
    ...
    # df_users = spark.createDataFrame(data=input_data, schema=input_columns)
    df_expected = spark.createDataFrame(data=expected_data, schema=expected_columns)
    assert_df_equality(bt.column_create(dataset_filtering_data, "multiplication", concat(col("id"), lit(' '), col("value"))), df_expected)
